...
def main(usr_args):
    ...
    ckpt_tag = Path(str(ckpt_setting)).name if ckpt_setting else "default"
    save_dir = Path(f"eval_result/{task_name}/{policy_name}/{task_config}/{ckpt_tag}/{current_time}")
...
def eval_policy(task_name,
    ...
    while succ_seed < test_num:
        ...
        episode_video_tmp = None
        if TASK_ENV.eval_video_path is not None:
            episode_video_tmp = f"{TASK_ENV.eval_video_path}/episode{TASK_ENV.test_num}--recording.mp4"
        ...
        if TASK_ENV.eval_video_path is not None:
            ...
            final_video_path = (
                f"{TASK_ENV.eval_video_path}/episode{TASK_ENV.test_num}--success={succ}.mp4"
            )
            if episode_video_tmp is not None and os.path.exists(episode_video_tmp):
                os.replace(episode_video_tmp, final_video_path)
